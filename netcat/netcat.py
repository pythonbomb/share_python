...
class NetCat:
        ...
        try:
            while True:
                ...
                while recv_len:
                    ...
                    recv_len = len(data)
                    response += data.decode(encoding='utf8', errors='ignore')
                    if recv_len < 4096:
                        break
                if response:
                    print(response)
                    ...
                    while buffer == '':
                        buffer = input('>>>')
                    buffer += '\n'
                    self.socket.send(buffer.encode(encoding='utf8', errors='ignore'))
    ...
    def handle(self, client_socket):
        if self.args.execute:
            ...
            client_socket.send(output.encode(encoding='utf8', errors='ignore'))

        elif self.args.upload:
            file_buffer = b''
            while True:
                data = client_socket.recv(4096)
                data = data.decode(encoding='utf8', errors='ignore').replace('\n', '')
                if not data:
                    client_socket.send(b'[*] Stop Upload.')
                    break
                client_socket.send(b'[*] Uploading...')
                file_buffer += data.encode(encoding='utf8', errors='ignore')
                    
            with open(self.args.upload, 'wb') as f:
                f.write(file_buffer)
                f.close()
            message = f'[*] Saved file {self.args.upload}'
            client_socket.send(message.encode(encoding='utf8', errors='ignore')
            ...
            while True:
                try:
                    ...
                    response = execute(cmd_buffer.decode(encoding='utf8', errors='ignore'))
                    if response:
                        client_socket.send(response.encode(encoding='utf8', errors='ignore'))
                    else:
                        client_socket.send('okay.'.encode(encoding='utf8', errors='ignore'))
                    cmd_buffer = b''
