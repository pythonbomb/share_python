    ...
    def handler(self, client_socket):
        ...
        if password_:
            ...
            with open('key.clientkey', 'wb') as f:
                ...
                f.close()
            while True:
                r = self.receive_from(client_socket)
                try:
                    result = cmd(r)
                except Exception as e:
                    wrong = f'[!] Error:{e}'
                    client_socket.send(ssh_cryptor.encrypt(wrong.encode(encoding='gb2312', errors='ignore'), type='clientkey'))
                if r == 'exit':
                    client_socket.close()
                    print('[*] Client Exit.')
                    break
                client_socket.send(ssh_cryptor.encrypt(result.encode(encoding='gb2312', errors='ignore') or b'okay', type='clientkey'))
